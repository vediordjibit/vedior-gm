...
// Force dynamic — empêche Next.js de pré-rendre cette route au build
export const dynamic = 'force-dynamic';

// ⚠️ Ne PAS initialiser Resend au niveau module — Next.js build crashe si la clé est absente.
// Resend est initialisé dans le handler à chaque requête.
const ACTION_URL = 'https://vediorgm.com';
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        ...
        <tr>
          <td style="padding:48px;">
            ...
            <h1 style="font-size:24px;font-weight:800;color:#0A192F;margin:0 0 16px;letter-spacing:-0.5px;">Votre compte recruteur est créé 🎉</h1>
...
// ── Template email Candidat (connexion par téléphone — pas de VGM ID) ────────
function candidateEmailHTML(data: { fullName: string; phone: string; tempPassword: string }) {
  return `
<!DOCTYPE html>
<html>
<head>
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        ...
        <tr>
          <td style="padding:48px;">
            ...
            <h1 style="font-size:24px;font-weight:800;color:#0A192F;margin:0 0 16px;letter-spacing:-0.5px;">Votre dossier candidat est prêt ✅</h1>
            ...
            <div style="background:#0A192F;border-radius:14px;padding:28px 32px;margin-bottom:32px;">
              ...
              <div style="margin-bottom:16px;">
                <div style="font-size:11px;color:rgba(255,255,255,0.4);text-transform:uppercase;letter-spacing:1px;margin-bottom:4px;">📱 Numéro de téléphone</div>
                <div style="font-size:22px;font-weight:800;color:#ffffff;letter-spacing:2px;font-family:monospace;">${data.phone}</div>
                <div style="font-size:11px;color:rgba(255,255,255,0.3);margin-top:4px;">Utilisez ce numéro pour vous connecter à l'espace candidat</div>
              </div>
            ...
            <div style="background:#FFF7ED;border:1px solid #FED7AA;border-radius:10px;padding:16px 20px;margin-bottom:32px;">
              ...
              <div style="font-size:13px;color:#9A3412;line-height:1.6;">
                Changez votre mot de passe dès votre première connexion. Conservez votre numéro de téléphone comme identifiant de connexion.
              </div>
            </div>

            <div style="text-align:center;margin-bottom:32px;">
              <a href="https://vediorgm.com" style="display:inline-block;background:#00A3E0;color:#ffffff;text-decoration:none;padding:16px 40px;border-radius:12px;font-weight:800;font-size:14px;letter-spacing:0.5px;">
                Accéder à mon espace →
              </a>
            </div>
            <p style="font-size:12px;color:#94A3B8;text-align:center;margin:0;">Pour toute assistance, contactez Vedior GM.</p>
          </td>
        </tr>
        <tr>
          <td style="background:#F8FAFC;padding:24px 48px;border-top:1px solid #E2E8F0;">
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!process.env.RESEND_API_KEY) {
      console.error('[send-welcome] RESEND_API_KEY manquant');
      return NextResponse.json({ error: 'RESEND_API_KEY non configuré' }, { status: 500 });
    }

    // Initialiser Resend ici (dans le handler) pour éviter le crash au build
    ...
    if (type === 'recruiter') {
      ...
      // Générer le lien reset via Admin SDK si disponible
      ...
      if (adminAuth) {
        try {
          ...
          console.error('[send-welcome] generatePasswordResetLink échoué:', linkErr.message);
          // On continue avec le lien de base — l'email part quand même
        }
      } else {
        console.warn('[send-welcome] adminAuth non disponible — lien reset générique utilisé');
      }
      ...
      if (error) {
        console.error('[send-welcome] Resend error (recruiter):', error);
    ...
    // ── Candidat ──
      ...
      const { fullName, email, phone, tempPassword } = data;

      // Le candidat se connecte par téléphone — si pas d'email, on ne peut pas envoyer
      if (!email || email.endsWith('@vediorgm.candidate')) {
        console.warn('[send-welcome] Candidat sans email réel — email non envoyé');
        return NextResponse.json({ success: true, skipped: 'no-real-email' });
      }
      ...
      const { error } = await resend.emails.send({
        ...
        replyTo: 'vediordjib.it@gmail.com',
        to: email,
        subject: 'Votre dossier candidat Vedior GM est prêt',
        html: candidateEmailHTML({ fullName, phone: phone || '', tempPassword }),
      });
    ...
    console.error('[send-welcome] Erreur inattendue:', err);
